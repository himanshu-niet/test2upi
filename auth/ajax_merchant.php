...
$sql = "SELECT * FROM `merchant` WHERE user_id='".$userAccount['user_id']."' and merchant_id='".$merchant_id."' ";
...
$merchant_upi = "HSBIMOPAD.{$response['Result'][0]['FinalTID']}-{$response['Result'][0]['MID']}@sbi";
...
$merchant = rechpay_fetch(rechpay_query("SELECT * FROM `merchant` WHERE user_id='".$userAccount['user_id']."' and merchant_id='".$merchant_id."' "));
...
$sql = "UPDATE `merchant` SET `merchant_primary`='InActive' WHERE user_id='".$userAccount['user_id']."' and merchant_id!='".$merchant['merchant_id']."'  ";	
if(rechpay_query($sql)){
